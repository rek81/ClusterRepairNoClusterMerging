...
LocalPoint PixelCPEGeneric::localPosition(DetParam const& theDetParam, ClusterParam& theClusterParamBase) const {
  ...
  int maxcol = theClusterParam.theCluster->maxPixelCol();
  ...
  std::cout << "min col is " << col_offset << " and max col is " << maxcol << std::endl;
  ...
  printf("next line will be cluster columns\n");
  for (unsigned long int i=0; i<collist.size(); i++){
    printf("%.0lu ", col_offset+i);
  }
  printf("\n");
  

  //  for (long unsigned int icol=0; icol<collist.size(); icol++){
  //    if (icol == collist[-1]){
  //      std::cout << icol << std::endl;
  //    }
  for (long unsigned int icol=0; icol<collist.size(); icol++){
    if (collist[icol]==0){
      std::cout << "found an all zero column" << std::endl;
    }
    //    else{
    //      std::cout << icol << " ";
    //    }
    ...
    //  for (long unsigned int icol=0; icol<collist.size(); icol++){
    //    if (icol == collist[-1]){
    //      std::cout << collist[icol] << std::endl;
    //    }
    //    else{
    //      std::cout << collist[icol] << " ";
    //    }
  }
